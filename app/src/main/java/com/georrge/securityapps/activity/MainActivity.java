...
import com.georrge.securityapps.contollers.Constants;
import com.georrge.securityapps.contollers.UsefulMethods;
...
public class MainActivity extends AppCompatActivity implements Constants, AdapterView.OnItemClickListener {
    ...
    private List<String> passOffApps = new LinkedList<>();      // list for un-locked apps
    ...
    * 12*/
    ...
    public void onEvent(MessageToMainActivity event){
        ...
        // added a app in in-locked list
        else if(event.code == 3){
            passOffApps.add(event.message);
        }
    }
    ...
    * */
    ...
    public void showPassOffListApps(View view) {
        Intent intent = new Intent(MainActivity.this, ListPassOff.class);
        int key = 0;
        for(String name_po : passOffApps)
            intent.putExtra("key_" + key++, name_po);
