...
public class MonitorAppsService extends IntentService {
    ...
    * */
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // EventBus.getDefault().post(new MessageToMainActivity(0, "MonitorAppsService_is_start")); ???? здесь явно нужно подумать
                ...
                for (String processName : listSecurityApps) {
                    ...
                    if (process.name.equals(processName)) {
                        if (process.foreground) {
                            ...
                            sleep(1000);    // for block to send several messages for once
                        }
                    }
                }
            }
